...
@interface DQPhotoLoader ()


@end


@implementation DQPhotoLoader

static DQPhotoLoader *sharePhotoLoader = nil;

+ (instancetype)sharePhotoLoader
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharePhotoLoader = [[self alloc] init];
    });
    return sharePhotoLoader;
}
...
+ (instancetype)allocWithZone:(struct _NSZone *)zone
{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharePhotoLoader = [super allocWithZone:zone];
    });
    return sharePhotoLoader;
}
...
{
    NSMutableArray<DQPhotoAlbumList *> *photoAblumList = [NSMutableArray array];
    ...
    [smartAlbums enumerateObjectsUsingBlock:^(PHAssetCollection * _Nonnull collection, NSUInteger idx, BOOL *stop) {
        ...
        if (!([collection.localizedTitle isEqualToString:@"Recently Deleted"] ||
            ...
            if (assets.count > 0) {
                DQPhotoAlbumList *ablum = [[DQPhotoAlbumList alloc] init];
